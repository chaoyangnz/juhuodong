...
//declare the plugin's version ; use to check the plugin exists
    $.calchooser = $.calchooser || {version:'0.1.0'};

//[--Plugin Define
    var calchooser = function(node,opts) {
...
//            },
        ...
        var _CAL = {
             ...
             * @param {object|string} vArg	可为日期对象或日期字符串，日期对象格式new Date(yyyy, mm, dd)，日期字符串格式yyyy-mm|m-dd|d
             * @function
        ...
        var _templateTime = [
            ...
            "<span>分钟</span>",
            "<dfn class=\"par\">",
            "<a href=\"javascript:;\" class=\"tDate_10\">\u5341</a>",
            "<a href=\"javascript:;\" class=\"tDate_10\">\u5206</a>",
        ...
        Calendar.prototype = {
            ...
            _init: function(config) {
                 ...
                 * @type String
                 */
                this.sMessage = config.sMessage || "<span>日期与时间</span>";
                /**
                 * 存放出发日期日历实例
                 ...
                 * @type Object
                 */
                this.triggerNode = config.triggerNode;//_CAL.$("#" + config.id.replace(/^#/, ""));    //
                /**
            ...
            _addEvent: function() {
                ...
                _CAL.on(obj, "click", function(e) {
                    ...
                    if(oTarget.tagName.toUpperCase() === "A" && oTarget.className != "disabled") {
                        //日期选中
                        //alert(oTarget.parentNode.className)
                        //alert(oTarget["data-date"])
                        //if(oTarget["data-date"]!=undefined)
                        //if(oTarget.parentNode.className!='par'){
                        that.onSelectDate(oTarget);
                        //}
                ...
                //日历为弹出显示时添加事件
                if(this.isPopup) {
//                    this.triggerNode.value = '年-月-日 时:分';
                    this.triggerNode.placeholder = '年-月-日 时:分'; //fix by jockyang
                    _CAL.on(this.triggerNode, "focus", function(e) {
                        var oTime = lzc.tagName('a',lzc.Class(lzc.id(obj.id),'ntime')[0]);
                        for(var len=oTime.length,i=0;i<len;i++)oTime[i].className = '';
                        that.empty(that);
            ...
            emptyTime: function(obj) {
                ...
                lzc.Class(lzc.id(obj.id),'calendar')[0].style.background = 'url(/assets/plugins/jquery.calchooser/timeBg.gif) no-repeat center';
            ...
            eTime: function(obj) {
                ...
                lzc.Class(lzc.id(obj.id),'calendar')[0].style.background = 'url(/assets/plugins/jquery.calchooser/oneTimeBg.gif) no-repeat center';
                lzc.Class(lzc.id(obj.id),'calendar')[0].style.width = '100%';
                lzc.Class(lzc.id(obj.id),'cal-container')[0].style.width = '250px';
...
//            id: 		   "#J_date_1",			//触发显示日历元素ID
                ...
                onSelectDate:  function(obj) {
                    if(obj["data-date"]) {
                        this.endDate = this.endDate = obj["data-date"];
                        this.render(this.endDate);
                        this.render(this.endDate);
                    } else {
                        this.oneMonthY(timeMonth);
                        this.endDate = oYear+'-'+lzc.zero(timeMonth)+'-'+this.r;
                    }
                },		//选择日期回调方法
                onClose:	   function() {
                    this.hideMessage();
    ...
    $.fn.calchooser = function(conf) {

        //return existing instance // let users can use the Public Methods
        //Usage: var obj = $('#id').calchooser({ <options> }).data("calchooser");
        var el = this.eq(typeof conf == 'number' ? conf : 0).data("calchooser");
        if (el) { return el; }
        ...
        this.each(function() {
            el = new calchooser(this, opts);
            $(this).data("calchooser", el);
        });
