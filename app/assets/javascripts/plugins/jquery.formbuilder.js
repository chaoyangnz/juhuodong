	...
	$.fn.formbuilder = function (options) {
		...
		var defaults = {
			...
			messages: {
				...
				text_field			: "单行文本",
				label				: "标签",
				paragraph_field		: "多行文字",
				select_options		: "选择项",
				add					: "添加选项",
				checkbox_group		: "复选栏位",
				remove_message		: "确定要删除此栏位吗?",
                remove_option_message: "确定要删除此选项吗?",
				remove				: "删除",
				radio_group			: "单选栏位",
				selections_message	: "允许多选",
				hide				: "隐藏",
				required			: "必输",
				show				: "显示"
			}
		...
		return this.each(function () {
            ...
            var controlBox = function(target) {
                ...
                var box_content = '<div id="' + box_id + '"  class="btn-group frmb-control">' +
                    '<button class="btn" for="input_text">单行文本</button>' +
                    '<button class="btn" for="textarea">多行文本</button>' +
                    '<button class="btn" for="checkbox">单选框</button>' +
                    '<button class="btn" for="radio">复选框</button>' +
                    '<button class="btn" for="select">下拉列表</button>' +
                    '</div>'
//                if (!target) {
//                    $(ul_obj).before(box_content);
//                } else {
                    $(target).prepend(box_content);
//                }

                $('#' + box_id + ' button').click(function () {
						appendNewField($(this).attr('for'));
//						$(this).val(0).blur();
...
//				}(opts.control_box_target);
			...
			var appendCheckboxGroup = function (values, options, required) {
					...
					field += '<div class="add-area"><a href="#" class="add add_ck"><i class="icon-plus-sign"></i>' + opts.messages.add + '</a></div>';
					field += '</div>';
					field += '</div>';
					help = '';
			...
			var checkboxFieldHtml = function (values) {
					...
					field += '<a href="#" class="remove" title="' + opts.messages.remove_option_message + '"><i class=" icon-minus-sign"></i>' + opts.messages.remove + '</a>';
					field += '</div>';
					return field;
				};
			...
			var appendRadioGroup = function (values, options, required) {
					...
					field += '<div class="add-area"><a href="#" class="add add_rd"><i class=" icon-plus-sign"></i>' + opts.messages.add + '</a></div>';
					field += '</div>';
					field += '</div>';
					help = '';
			...
			var appendSelectList = function (values, options, required) {
					...
					field += '<div class="add-area"><a href="#" class="add add_opt"><i class=" icon-plus-sign"></i>' + opts.messages.add + '</a></div>';
					field += '</div>';
					field += '</div>';
					help = '';
			...
			var appendFieldLi = function (title, field_html, required, help) {
					...
					li += '<div class="legend">';
					li += '<a id="frm-' + last_id + '" class="toggle-form open" href="#"><i class="icon-chevron-up"></i>' + opts.messages.hide + '</a> ';
					li += '<a id="del_' + last_id + '" class="del-button delete-confirm" href="#" title="' + opts.messages.remove_message + '"><i class="icon-minus-sign"></i><span>' + opts.messages.remove + '</span></a>';
			...
			$('ul.frmb .remove').live('click', function () {
			...
			$('ul.frmb  .toggle-form').live('click', function () {
				var target = $(this).attr("id");
				if ($(this).hasClass('open')) {
					$(this).removeClass('open').addClass('closed').html('<i class="icon-chevron-down"></i>' + opts.messages.show);
					$('#' + target + '-fld').animate({
						opacity: 'hide',
						height: 'hide'
					}, 'slow');
				...
				if ($(this).hasClass('closed')) {
					$(this).removeClass('closed').addClass('open').html('<i class="icon-chevron-up"></i>' + opts.messages.hide);
					$('#' + target + '-fld').animate({
						opacity: 'show',
						height: 'show'
					}, 'slow');
			...
			$('ul.frmb .delete-confirm').live('click', function () {
			...
			$('ul.frmb .add_ck').live('click', function () {
				$(this).parent().before(checkboxFieldHtml());
				return false;
			});
			// Attach a callback to add new options
			$('ul.frmb .add_opt').live('click', function () {
				$(this).parent().before(selectFieldHtml('', false));
				return false;
			});
			// Attach a callback to add new radio fields
			$('ul.frmb .add_rd').live('click', function () {
 ...
 */
(function ($) {
    $.fn.serializeJsonFormList = function (options) {
        // Extend the configuration options with user-provided
        var defaults = {
            prepend: 'ul',
            is_child: false,
        ...
        this.each(function () {
            ...
            $(this).children().each(function () {
                ...
                for (att = 0; att < opts.attributes.length; att++) {
                    ...
                    field[key] = encodeURIComponent($(this).attr(opts.attributes[att]))
                    // append the form field values
                    if (opts.attributes[att] === 'class') {
                        field.required = encodeURIComponent($('#' + $(this).attr('id') + ' input.required').attr('checked') ? true : false)
                        switch ($(this).attr(opts.attributes[att])) {
                            case 'input_text':
                                field.title = encodeURIComponent($('#' + $(this).attr('id') + ' input[type=text]').val());
                                break;
                            case 'textarea':
                                field.title = encodeURIComponent($('#' + $(this).attr('id') + ' input[type=text]').val());
                                break;
                            case 'checkbox':
                                ...
                                $('#' + $(this).attr('id') + ' input[type=text]').each(function () {
                                    ...
                                    if ($(this).attr('name') === 'title') {
                                        field.title = encodeURIComponent($(this).val());
                                    }
                                    else {
                                        field.options.push({value: encodeURIComponent($(this).val()), checked: $(this).prev().attr('checked') ? true : false})
                                    }
                                    c++;
                                });
                                break;
		...
		this.each(function () {
			...
			$(this).children().each(function () {
				for (att = 0; att < opts.attributes.length; att++) {
					...
					// append the form field values
					if (opts.attributes[att] === 'class') {
						serialStr += opts.prepend + '[' + li_count + '][required]=' + encodeURIComponent($('#' + $(this).attr('id') + ' input.required').attr('checked') ? true : false);
						switch ($(this).attr(opts.attributes[att])) {
						case 'input_text':
						...
						case 'checkbox':
							...
							$('#' + $(this).attr('id') + ' input[type=text]').each(function () {
								if ($(this).attr('name') === 'title') {
									...
									serialStr += opts.prepend + '[' + li_count + '][options][' + c + '][checked]=' + ($(this).prev().attr('checked') ? true : false);
								}
								c++;
							});
							break;
